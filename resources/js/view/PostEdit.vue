            ...
            <PageHeader title="Post Editer">
                <template v-slot:status>
                    <ul class="navbar-nav ms-auto pe-3">
                        <li class="nav-item">
                            <a class="btn nav-link" @click.prevent="savePost">{{
                                status
                            }}</a>
                        </li>
                    </ul>
        ...
        fetchPost() {
            return this.request()
                ...
                .then(({ data }) => {
                    ...
                    this.post.published_at = get(
                        data.post,
                        "published_at",
                        null
                    );
                    this.post.featured_image = get(
                        data.post,
                        "featured_image",
                        ""
                    );
                    this.post.meta.title = get(data.post.meta, "title", "");
                    this.post.meta.description = get(
                        data.post.meta,
                        "description",
                        ""
                    );
                    this.post.meta.keywords = get(
                        data.post.meta,
                        "keywords",
                        ""
                    );
                    ...
                    this.status =
                        get(data.post, "published_at", null) !== null
                            ? "Update"
                            : "Save";
                })
